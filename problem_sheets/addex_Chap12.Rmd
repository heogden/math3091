...
$f_{Y}(y; \lambda) = \lambda^2 y e^{-2 y}$, $y > 0$,
$\lambda > 0$.
 \benum
 \item Show that the $\text{gamma}(2, \lambda)$ distribution is a member
  of the exponential family, and hence find $E(Y)$
  ...
  Find a log-likelihood ratio test statistic $L_{01}$ which could be used to test this hypothesis.
...
\benum
\item Derive
    the canonical link function for the gamma distribution.
...
\begin{equation}
\eta_i = \beta_1 + \beta_2 x_{i}
\label{eq:mod_linear}
\end{equation}
and that $\bm \beta = (\beta_1, \beta_2)$.
Assuming the canonical link, write down the likelihood in terms of $\beta_1$ and $\beta_2$
...
\item Calculate the observed information matrix $-H(\bm \beta)$
...
\item \ Consider
    the alternative model $Y_i \sim \text{Gamma}(2, \lambda_i)$,
  where
\begin{equation}
...
\item Someone suggests using the link function $g(\mu) = \sqrt{\mu}$
instead of the canonical link. Do you think this would be a sensible choice?
Justify your answer.
\eenum
