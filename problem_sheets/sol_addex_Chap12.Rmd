...
so 
\[P(z_{0.015} \leq \frac{\sqrt{2n}(\hat \lambda - \lambda)}{\lambda} \leq z_{0.985}) \approx 0.97,\] 
or
\[P(\hat \lambda - \frac{\lambda}{\sqrt{2 n}}z_{0.985}\leq \lambda \leq \hat \lambda - \frac{\lambda}{\sqrt{2 n}}z_{0.015}) \approx 0.97.\]
Replacing $\lambda$ with $\hat \lambda$ in the two endpoints
\[  \left[\hat \lambda - \frac{\hat \lambda}{\sqrt{2 n}}z_{0.985}, \hat \lambda - \frac{\hat \lambda}{\sqrt{2 n}}z_{0.015}\right]\]
is a $97\%$ confidence interval for $\lambda$, where
$z_{0.015} = - z_{0.985}$, and $z_{0.985}$ is
```{r}
qnorm(0.985)
```
...
\end{align*}
   The Fisher scoring algorithm starts with an initial estimate
    $\bm{\beta}^{(0)}$ of $\bm{\beta}$. If $\bm{\beta}^{(m)}$
    is the current estimate of $\hat{\bm{\beta}}$
then the next estimate is
\[\bm{\beta}^{(m+1)}=\bm{\beta}^{(m)}+{\cal I}(\bm{\beta}^{(m)})^{-1}\bm{u}(\bm{\beta}^{(m)}).\]
The process stops once
$||\bm{\beta}^{(m+1)}-\bm{\beta}^{(m)} || \leq \epsilon$, for some
pre-defined tolerance $\epsilon$, and we use
$\bm{\beta}^{(m+1)}$ as the solution for $\hat{\bm{\beta}}$.

The Fisher and observed information matrices are
different in this case, so the Fisher scoring Newton-Raphson methods are not identical.
