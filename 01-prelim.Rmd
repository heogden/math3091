...
absorbing liquid (`conc`). Figure \@ref(fig:nitric) gives a plot
of the data.

```{r nitric, echo = FALSE, fig.pos = "tb", fig.cap = "A pairs plot of the `nitric` data"}
nitric <- read.csv("nitric.csv")
pairs(nitric)
```

These data will be analysed  in worksheet 2 using multiple linear 
regression models.   
...
depends on sex and  gestational age. Figure \@ref(fig:birth) gives a plot
of the data.

```{r birth, echo = FALSE, fig.pos = "tb", fig.cap = "The `birth` data"}
...
This data set will be analysed  in
...
of  3 poisons and 4 treatments. The aim here is to study how survival time depends
...
It seems sensible to model the number of beetles 
...
\end{align}
where $\epsilon_i \sim N(0, \sigma^2)$ independently, and
\[\bm{X}= \begin{pmatrix}
\bm{X}_1^T \\
\vdots \\
\bm{X}_n^T
...
We note that $\bm{\epsilon}$ has a multivariate normal distribution
...
We may write the linear model more compactly by noting that 
...
so
\[\hat{Y}_i=\bm{X}_i^T\hat{\bm{\beta}}\qquad i = 1, \ldots, n.\]
...
and fitted values
\[\hat{\bm{\epsilon}}=\bm{y}-\bm{X}\hat{\bm{\beta}}\]
so
\[\hat{\epsilon}_i=y_i-\bm{X}_i^T\hat{\bm{\beta}}\qquad i = 1, \ldots, n.\]
...
We call
\[D= \sum_{i=1}^n \hat{\epsilon}_i^2 = \sum_{i=1}^n \left(y_i-\bm{X}_i^T\hat{\bm{\beta}}\right)^2\]
...
We can estimate $\sigma^2$ using maximum likelihood. Maximising \@ref(eq:lmLikelihood)
