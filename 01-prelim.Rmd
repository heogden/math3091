...
When $f_{\bm Y}(\bm y;\bm \theta)$ is considered as a
function of $\bm \theta$ for fixed (observed) $y_1, \ldots, y_n$, we call it the
*likelihood function*.
```
...
* Often the likelihood function is written $L(\bm \theta)$ or $L(\bm \theta;\bm{y})$.
* Frequently it is more convenient to consider the log-likelihood function
  $\log L(\bm \theta)$. This is often denoted by $\ell(\bm \theta)$ or $\ell(\bm \theta;\bm y)$.
* Nothing in the definition of the likelihood requires $y_1, \ldots, y_n$ to be
  observations of independent random variables, although we shall frequently
  make this assumption.
* Any factors which depend on $y_1, \ldots, y_n$ alone (and not on $\bm \theta$) can be ignored
...
function, the value of $\bm \theta$ which maximises $L(\theta)$ also maximises $\ell(\theta)$.
It is almost always easier to maximise $\ell(\theta)$.
This is achieved in the usual way; finding a stationary point by differentiating
$\ell(\theta)$ with respect to $\theta_1, \ldots, \theta_p$, and solving the resulting $p$
...
\begin{eqnarray*}
&\qquad \ell(p)=n\bar y\log p+n(1-\bar y)\log(1-p)\cr
&\;\>\,{\partial\over{\partial p}}\ell(\theta){p}=
{{n\bar y}\over p}-{{n(1-\bar y)}\over{1-p}}\cr
&\Rightarrow\quad 0=
{{n\bar y}\over {\hat{p}}}-{{n(1-\bar y)}\over{1-\hat{p}}}\cr
&\Rightarrow\quad \hat{p}=\bar y.
\end{eqnarray*}
Note that ${{\partial^2}\over{\partial p^2}}\ell(\theta){p}=
{{-n\bar y}/p^2}-{{n(1-\bar y)}/({1-p})^2}<0$ everywhere, so the stationary point is
clearly a maximum.
...
Here $\bm \theta=(\mu,\sigma^2)$ and
\begin{eqnarray*}
&\ell(\theta){\mu,\sigma^2}=-{n\over 2}\log(2\pi)-{n\over 2}\log(\sigma^2)
-{1\over{2\sigma^2}}\sum(y_i-\mu)^2\cr
&{\partial\over{\partial \mu}}\ell(\theta){\mu,\sigma^2}=
{1\over{\sigma^2}}\sum(y_i-\mu)={{n(\bar y-\mu)}\over{\sigma^2}}\cr
&\quad\Rightarrow\quad 0={{n(\bar y-\hat{\mu})}\over{\sigma^2h}}& (1) \cr
&{\partial\over{\partial \sigma^2}}\ell(\theta){\mu,\sigma^2}=
-{n\over {2\sigma^2}}+{1\over{2(\sigma^2)^2}}\sum(y_i-\mu)^2\cr
&\quad\Rightarrow\quad 0=
-{n\over {2\sigma^2h}}+{1\over{2(\sigma^2h)^2}}\sum(y_i-\hat{\mu})^2& (2)
\end{eqnarray*}
Solving $(1)$ and $(2)$, we obtain
\begin{eqnarray*}
\hat{\mu}&=&\bar y\cr
\sigma^2h&=&{1\over n}\sum(y_i-\hat{\mu})^2=
...
Here
$$
\bm{H}(\hat{\mu},\sigma^2h)= \left( \begin{array}{cc}
- \frac{n}{\sigma^2h } & 0 \cr
0   &-\frac{n}{2(\sigma^2h)^2} \end{array} \right)
$$
...
$$
u(\theta)\equiv{{\partial}\over{\partial\theta}}\log L(\theta){\theta}.
$$
...
\begin{eqnarray*}
E[U_i(\bm \theta)]&=&\int U_i(\bm \theta)L(\theta) d\bm y\cr
&=& \int {{\partial}\over{\partial\theta_i}} \ell(\theta) L(\theta) d\bm y\cr
&=& \int {{{{\partial}\over{\partial\theta_i}}L(\theta)}\over L(\theta)} L(\theta) d\bm y\cr
&=& \int {{\partial}\over{\partial\theta_i}}L(\theta) d\bm y\cr
&=& {{\partial}\over{\partial\theta_i}}\int L(\theta) d\bm y\cr
&=& {{\partial}\over{\partial\theta_i}} 1 =0\qquad i=1,\ldots p.\cr
\end{eqnarray*}
...
\begin{eqnarray*}
u(p)=n\bar y/ p-n(1-\bar y)/(1-p)\cr
...
u_2(\mu,\sigma^2)&=& -{n\over {2\sigma^2}}+{1\over{2(\sigma^2)^2}}\sum(y_i-\mu)^2\cr
E[\bm{u}(\mu,\sigma^2)]={\bf 0}\quad&\Rightarrow & \quad E[\bar y]=\mu\quad{\rm and}\quad
...
scalar defined as
$$
-H(\theta)\equiv-{{\partial}\over{\partial\theta^2}}\log L(\theta){\theta}.
$$
...
$$
[\mathcal{I}]_{ij}=E(-[H(\bm{\theta})]_{ij})
\qquad i=1,\ldots ,p;\;j=1,\ldots ,p.
$$

An important result in likelihood theory is that the variance-covariance matrix
of the score vector is equal to the
expected information matrix {\it i.e.}
$$
\text{Var}[U(\bm{\theta})]=\mathcal{I}\quad \Leftrightarrow \quad
\text{Var}[U(\bm{\theta})]_{ij}= [\mathcal{I}]_{ij}
\qquad i=1,\ldots ,p;\;j=1,\ldots ,p,
$$
provided that
...
{{\partial}\over{\partial\theta_j}} \ell(\theta) L(\theta) d\bm y\cr
&=& \int {{{{\partial}\over{\partial\theta_i}}L(\theta)}\over L(\theta)}
{{{{\partial}\over{\partial\theta_j}}L(\theta)}\over L(\theta)} L(\theta) d\bm y\cr
&=& \int \frac{1}{L(\theta)}{{\partial}\over{\partial\theta_i}}L(\theta)
 {{\partial}\over{\partial\theta_j}}  L(\theta)  d\bm y \cr
& & \hskip 1.2in i=1,\ldots ,p;\;j=1,\ldots ,p.
\end{eqnarray*}
Now
\begin{eqnarray*}
[\mathcal{I}]_{ij}&=&E\left[-{{\partial^2}\over
{\partial\theta_i\partial\theta_j}} \ell(\theta)\right]\cr
&=&\int -{{\partial^2}\over
{\partial\theta_i\partial\theta_j}} \ell(\theta)L(\theta) d\bm y\cr
&=&\int -{{\partial}\over{\partial\theta_i}}\left[
{{{{\partial}\over{\partial\theta_j}}L(\theta)}\over L(\theta)}\right]
L(\theta) d\bm y\cr
&=&\int \left[
-{{{{\partial^2}\over{\partial\theta_i\partial\theta_j}}L(\theta)}\over L(\theta)}
+ {{{{\partial}\over{\partial\theta_i}}L(\theta)
{{\partial}\over{\partial\theta_j}}L(\theta)}\over L(\theta)^2} \right]
L(\theta) d\bm y\cr
&=& -{{\partial^2}\over{\partial\theta_i\partial\theta_j}}\int L(\theta) d\bm y
+ \int \frac{1}{L(\theta)}{{\partial}\over{\partial\theta_i}}L(\theta)
{{\partial}\over{\partial\theta_j}}L(\theta)  d\bm y\cr
&=& \text{Var}[U(\bm{\theta})]_{ij}\qquad i=1,\ldots ,p;\;j=1,\ldots ,p.
\end{eqnarray*}
...
$\hat{\theta}_i$ has a
$N(\theta_i,[\mathcal{I}^{-1}]_{ii})$ distribution and we can find $z_{1-\frac{\alpha}{2}}$ such that
$$
P\left(z_{1-\frac{\alpha}{2}}\le {{\hat{\theta}_i-\theta_i}\over{[\mathcal{I}^{-1}]_{ii}^{1\over 2}}}\le
z_{1-\frac{\alpha}{2}}\right) = 1- \alpha.
$$
Therefore
$$
P\left(\hat{\theta}_i-z_{1-\frac{\alpha}{2}}[\mathcal{I}^{-1}]_{ii}^{1\over 2}\le\theta_i
\le\hat{\theta}_i+z_{1-\frac{\alpha}{2}}[\mathcal{I}^{-1}]_{ii}^{1\over 2}
\right) = 1- \alpha.
$$
The endpoints of this interval cannot be evaluated
because they also depend on the unknown parameter vector $\bm \theta$.
However, if we replace $\mathcal{I}$ by its MLE ${\cal I}(\hat{\bm \theta})$
...
of the form
\[C=\left\{ \bm{y}: 
\frac{\max_{\bm \theta\in \Theta^{(1)}}f_{\bm Y}(\bm{y};\bm \theta)}
{\max_{\bm \theta\in \Theta^{(0)}}f_{\bm Y}(\bm{y};\bm \theta)} 
>k\right\}\]
...
<!-- \begin{eqnarray} -->
<!-- &  {{\max_{p\in(0,1)}f_{\bm Y}(\bm{y};p)}\over{\max_{p=p_0}f_{\bm Y}(\bm{y};p)}} > & k \nonumber  \cr -->
<!-- \Rightarrow \quad &  {{\bar y^{\Sigma y_i}(1-\bar y)^{n-\Sigma y_i}}\over {p_0^{\Sigma y_i}(1-p_0)^{n-\Sigma y_i}}} > & k \nonumber \cr -->
<!-- \Rightarrow\quad &  \left({{\bar y}\over{p_0}}\right)^{n\bar y}  -->
<!-- \left({{1-\bar y}\over{1-p_0}}\right)^{n(1-\bar y)} & >k. \label{her1}\cr -->
<!-- \end{eqnarray} -->
<!-- Now the left hand side of (\ref{her1}), is minimised as a function of $\bar y$ -->
<!-- at $\bar y=p_0$ and increases as -->
<!-- $\bar y$ moves away from $p_0$ in either direction. -->
<!-- Therefore, the rejection region (\ref{her1}) is equivalent to -->
<!-- $$ -->
<!-- C=\left\{ \bm{y}:\bar y > k'\;{\rm or}\; \bar y < k''\right\} -->
<!-- $$ -->
...
of the form
$$
C=\left\{ \bm{y}:{{\max_{\bm \theta\in \Theta^{(1)}}f_{\bm Y}(\bm{y};\bm \theta)}\over
{\max_{\bm \theta\in \Theta^{(0)}}f_{\bm Y}(\bm{y};\bm \theta)}} >k\right\}
$$
where $k$ is determined by $\alpha$, the size of the test, so
$$
\max_{\bm \theta\in\Theta^{(0)}}P(\bm{y}\in C;\bm \theta)=\alpha.
...
$$
C=\left\{ \bm{y}: 2\log \left({{\max_{\bm \theta\in \Theta^{(1)}}f_{\bm Y}(\bm{y};\bm \theta)}\over
{\max_{\bm \theta\in \Theta^{(0)}}f_{\bm Y}(\bm{y};\bm \theta)}}\right) >k'\right\}
$$
where
$$
\max_{\bm \theta\in\Theta^{(0)}}P(\bm{y}\in C;\bm \theta)=\alpha.
...
$n\to\infty$
$$
L_{01}\equiv 2\log \left({{\max_{\bm \theta\in \Theta^{(1)}}f_{\bm Y}(\bm{y};\bm \theta)}\over
{\max_{\bm \theta\in \Theta^{(0)}}f_{\bm Y}(\bm{y};\bm \theta)}}\right)
$$
...
a Taylor series expansion of $\log f_{\bm Y}(\bm{y};\theta)$  around the
MLE $\hat{\theta}$ gives
$$
\log f_{\bm Y}(\bm{y};\theta)=\log f_{\bm Y}(\bm{y};\hat{\theta})+(\theta-\hat{\theta})
...
terms, we obtain
$$
2[\log f_{\bm Y}(\bm{y};\hat{\theta})-\log f_{\bm Y}(\bm{y};\theta)]=
(\theta-\hat{\theta})^2 {\cal I}(\theta)
$$
As $\hat{\theta}$ is asymptotically
$N[\theta,{\cal I}(\theta)^{-1}]$, $(\theta-\hat{\theta})^2 {\cal I}(\theta)$ is
asymptotically $\chi^2_1$, and hence so is $2[\log f_{\bm Y}(\bm{y};\hat{\theta})-\log
f_{\bm Y}(\bm{y};\theta)]$.

Similarly it can be shown that when $\bm \theta\in\Theta$, a multidimensional space,
$2[\log f_{\bm Y}(\bm{y};\hat{\bm \theta})-\log f_{\bm Y}(\bm{y};\bm \theta)]$ is asymptotically
...
$\bm \theta\in\Theta^{(1)}$. Furthermore, suppose that $\log f_{\bm Y}(\bm{y};\hat{\bm \theta})$
is maximised in $\Theta^{(0)}$ by $\hat{\bm \theta}^{(0)}$ and is maximised
in $\Theta^{(1)}$ by $\hat{\bm \theta}^{(1)}$. Then
\begin{eqnarray*}
L_{01}&\equiv&  2\log \left({{\max_{\bm \theta\in \Theta^{(1)}}f_{\bm Y}(\bm{y};\bm \theta)}\over
{\max_{\bm \theta\in \Theta^{(0)}}f_{\bm Y}(\bm{y};\bm \theta)}}\right)\cr
&=& 2\log f_{\bm Y}(\bm{y};\hat{\bm \theta}^{(1)})-2\log f_{\bm Y}(\bm{y};\hat{\bm \theta}^{(0)})\cr
&=& 2[\log f_{\bm Y}(\bm{y};\hat{\bm \theta}^{(1)})-\log f_{\bm Y}(\bm{y};\bm \theta)]
 -2[\log f_{\bm Y}(\bm{y};\hat{\bm \theta}^{(0)})-\log f_{\bm Y}(\bm{y};\bm \theta)]\cr
&=& L_1-L_0.
\end{eqnarray*}
...
&= & [\bm{X}\bm{\beta}]_i,\qquad i=1,\ldots ,n \hspace{2cm} (1)
\end{eqnarray*}
where
$$
...
The four ways of describing the linear model in (1) are equivalent, but the most
economical is the matrix form
$$
E(\bm Y)=\bm{X}\bm{\beta}. \eqno{(2)}
$$
...
- $\text{Var}(Y_1)=\text{\text{Var}}(Y_2)=\cdots =\text{Var}(Y_n)$ ($Y_1, \ldots, Y_n$ are homoscedastic).
...
$$
\bm Y=\bm{X}\bm{\beta}t+\bm{\epsilon} \eqno{(3)}
$$
...
implies all $Cov(\epsilon_i,\epsilon_j)=0$).
It follows from (3) that the distribution of $\bm Y$ is multivariate normal
...
$\bm{\beta}$ to make the fitted values $\hat{E}(\bm Y)=\bm{X}\hat{\bm{\beta}}$ *as close as possible* to the
...
&=&\sum_{i=1}^n \left(y_i-\sum_{j=1}^p x_{ij}\beta_j\right)^2\cr
%&=\sum_{i=1}^n \left(y_i-[\bm{X}\bm{\beta}]_i\right)^2\cr
%&=(\bm{y}-\bm{X}\bm{\beta})^T(\bm{y}-\bm{X}\bm{\beta})
\end{eqnarray*}
as a function of $\beta_1, \ldots, \beta_p$.
The sum of squares may also be written as $(\bm{y}-\bm{X}\bm{\beta})^T(\bm{y}-\bm{X}\bm{\beta})$.

Differentiating w.r.t. $\beta_k,\;k=1,\ldots ,p$, and setting equal to 0 gives
\begin{eqnarray*}
 \qquad -2 \sum_{i=1}^n x_{ik}\left(y_i-\sum_{j=1}^p x_{ij}\hat{\beta}_j\right)
& = &   0\qquad k=1,\ldots ,p\cr
\Rightarrow  \qquad
\sum_{i=1}^n x_{ik}y_i & = & \sum_{i=1}^n\sum_{j=1}^p x_{ik}x_{ij}\hat{\beta}_j
\qquad k=1,\ldots ,p\cr
\Rightarrow  \qquad
[\bm{X}^T\bm{y}]_k & = & [\bm{X}^T\bm{X}\hat{\bm{\beta}}]_k\cr
\Rightarrow  \qquad
\bm{X}^T\bm{y} & = & \bm{X}^T\bm{X}\hat{\bm{\beta}}\cr
\end{eqnarray*}
...
\begin{eqnarray*}
& & \qquad\hat{E}(\bm Y)=\bm{X}\hat{\bm{\beta}}=\bm{X}(\bm{X}^T\bm{X})^{-1}\bm{X}^T\bm{y}\cr
\Rightarrow & & \qquad \hat{E}(Y_i)=\bm{X}_i^T\hat{\bm{\beta}}\qquad i = 1, \ldots, n.
\end{eqnarray*}
...
We can estimate $\sigma^2$ using maximum likelihood. Maximising (4)
with respect to $\bm{\beta}$ and $\sigma^2$ gives
$$
\sigma^2h={D\over n}={1\over n}\sum_{i=1}^n \hat{\epsilon}_i^2.
$$
...
$$
\sigma^2t={D\over {n-p}}={1\over {n-p}}\sum_{i=1}^n \hat{\epsilon}_i^2.
$$
...
$$
C=\left\{ \bm{y}:{{\max_{(\bm{\beta},\sigma^2)\in \Theta^{(1)}}f_{\bm Y}(\bm{y};\bm{\beta},\sigma^2)}\over
{\max_{(\bm{\beta},\sigma^2)\in \Theta^{(0)}}f_{\bm Y}(\bm{y};\bm{\beta},\sigma^2)}} >k\right\}
$$
where $k$ is determined by $\alpha$, the size of the test, so
$$
...
Therefore
\begin{eqnarray*}
\max_{\bm{\beta},\sigma^2} f_{\bm Y}(\bm{y};\bm{\beta},\sigma^2)&=&(2\pi D/n)^{-{n\over 2}}
...
$$
\text{Cook's distance}=
{{(\hat{\bm{\beta}}'-\hat{\bm{\beta}})^T\bm{X}^T\bm{X}(\hat{\bm{\beta}}'-\hat{\bm{\beta}})}\over{p\sigma^2h}},
$$
