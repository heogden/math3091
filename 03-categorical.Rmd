...
```{r, echo = FALSE}
library(knitr)
job <- read.csv("datasets/job.csv", stringsAsFactors = FALSE)
...
Income (\$)  & Very Dissatisfied & A Little Dissatisfied & Moderately Satisfied & Very Satisfied\\
\midrule
<6000 & 20 & 24 & 80 & 82\\
...
The original data structure of the `job` dataset allows
us to fit a log-linear model for the contitency table
```{r, echo = FALSE}
job <- read.csv("datasets/job.csv")
knitr::kable(job, format = "markdown")
```
...
The original data structure of the `lymphoma` dataset allows
us to fit a log-linear model for the contitency table
```{r, echo = FALSE}
lymphoma <- read.csv("datasets/lymphoma.csv")
knitr::kable(lymphoma, format = "markdown")
```
