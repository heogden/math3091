...
This log-linear model may be fitted in `R` with
```{r}
fit <- glm(Count ~ Remis * Cell + Cell * Sex, data = lymphoma, 
           family = poisson)
```
The estimated cell means may be extracted and converted to probabilities
```{r}
cell_means <- fitted(fit)
N <- sum(lymphoma$Count)
cell_probs <- cell_means/N
```
These cell probabilities are shown in Table \ref{tab:lymphoma_p}
