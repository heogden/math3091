...
variables.
For example, the dataset `job` represents the cross-classification of 901
individuals according to their income (4 levels) and job satisfaction
(4 levels).

```{r}
job <- data.frame()
```
Could put this in a table in markdown.
But better would be to read the data into `R` directly.
Can I get more recent data?
\begin{tabular}{lcccc}
\hline \\
& \multicolumn{4}{c}{Job Satisfaction} \\ \cline{2-5}
% & \multicolumn{4}{c}{\hline} \\
& Very & Little & Moderately & Very \\
Income (US\$) & Dissatisfied & Dissatisfied & Satisfied & Satisfied \\ \hline
$<$6000  & 20 & 24 & 80 & 82 \\
6000--15000 & 22 & 38 & 104 & 125 \\
15000--25000 & 13 & 28 & 81 & 113 \\
$>$25000 & 7 & 18 & 54 & 92 \\ \hline
\end{tabular}
*Source:* 1984 General Social Survey.

A cross-classification table, like the one above is called a *contingency table*. 
This is a *two-way table*, as there are two classifying variables.
We also describe the table above as a $4\times 4$ contingency table.
...
The  above `lymphoma` dataset, representing 30 lymphoma
patients classified by cell type of lymphoma, sex and response to treatment, is an example of a
...
margins of the table as below.

\begin{tabular}{lcccc|c}
\hline \\
& \multicolumn{4}{c}{Job Satisfaction} &  \\ \cline{2-5}
& Very & Little & Moderately & Very & \\
Income (US\$) & Dissatisfied & Dissatisfied & Satisfied & Satisfied & Total \\ \hline
$<$6000  & 20 & 24 & 80 & 82 & {\bf 206} \\
6000--15000 & 22 & 38 & 104 & 125 & {\bf 289} \\
15000--25000 & 13 & 28 & 81 & 113 & {\bf 235} \\
$>$25000 & 7 & 18 & 54 & 92 & {\bf 171}  \\ \hline
Total & {\bf 62}  & {\bf 108} &   {\bf 319} &   {\bf 412} & {\bf 901} \\ \hline
\end{tabular}

These are one-way margins as they represent the classification of items by
a single variable; Income group and Response respectively.
...
\noalign{\hrule}
\noalign{\vskip 8 pt}
&\multispan2\hfill \qquad Cell Type\hfill\cr
\noalign{\vskip -4 true pt}
Sex&Nodular&Diffuse\cr
\noalign{\vskip 8 pt}
\noalign{\hrule}
\noalign{\vskip 8 pt}
Male&5&13\cr
...
Cell(i) & income & satis & count ($y_i$) \\ \hline
 1 &  $<$6000  &  Dissatis  &   20 \\
...
&\hfill # \quad\hfill&\hfill # \quad\hfill&\hfill # \hfill\cr
\noalign{\hrule}
\noalign{\vskip 8 pt}
&&\multispan3\hfill Explanatory variables\hfill\cr
\noalign{\vskip -7pt}
Cell ($i$)&Response ($y_i$)\cr
\noalign{\vskip -7pt}
&&Cell Type&Sex&Remission\cr
\noalign{\vskip 8 pt}
\noalign{\hrule}
\noalign{\vskip 8 pt}
1&1&Nodular&Male&No\cr
2&2&Nodular&Female&No\cr
3&12&Diffuse&Male&No\cr
4&3&Diffuse&Female&No\cr
5&4&Nodular&Male&Yes\cr
6&6&Nodular&Female&Yes\cr
7&1&Diffuse&Male&Yes\cr
8&1&Diffuse&Female&Yes\cr
\noalign{\vskip 8pt}
\noalign{\hrule}
}}$$
...
for the purposes of modelling, it is often untenable, as a result of
...
$$\begin{array}{lcl}
f_{\bm{Y}}(\bm{y};{\bf p})&=&P(Y_1=y_1,\ldots ,Y_n=y_n)\cr
&=& \left\{\begin{array}{ll}
N!\prod_{i=1}^n {{p_i^{y_i}}\over{y_i!}} & \mbox{if }
\displaystyle\sum_{i=1}^n y_i=N\cr
0&\mbox{otherwise} \  \ \ \ \ \  \ \  (1)
\end{array}
\right.
\end{array}$$
...
$$
f_{\bm{Y}}(\bm{y};\bm{\mu})=\prod_{i=1}^n {{e^{-\mu_i} \mu_i^{y_i}}\over{y_i!}}.
$$
Therefore,
$$\begin{array}{ll}
\log f_{\bm{Y}}(\bm{y};\bm{\mu})&=-\sum_{i=1}^n \mu_i
+\sum_{i=1}^ny_i\log\mu_i-\sum_{i=1}^n \log y_i! \ \ \ \ \ \  (2)\cr
...
$$
\log\mu_i=\alpha + {\rm  other terms depending on} i,\qquad {i=1,\ldots ,n}
$$
so that
$$\begin{array}{ll}
&\qquad{\partial\over{\partial\alpha}}\log f_{\bm{Y}}(\bm{y};\bm{\mu})=-\sum_{i=1}^n \exp(\log\mu_i)
+\sum_{i=1}^ny_i.\cr
&\Rightarrow\qquad \sum_{i=1}^n \hat{\mu}_i=\sum_{i=1}^n y_i. \ \ \ \ \ \ \  (3)
\end{array}
$$
From (2), we notice that, at $\alpha=\hat{\alpha}$ the log likelihood takes the
form
$$
\log f_{\bm{Y}}(\bm{y};{\bm{\mu}})=-\sum_{i=1}^n y_i
...
maximising
$\sum_{i=1}^ny_i\log{\mu}_i$ subject to the constraints $\sum_{i=1}^n
\mu_i=\sum_{i=1}^n y_i$ from (3) and $\log\bm{\mu}=\bm{X}\bm{\beta}$
...
the patients was fixed in advance to be
\small
$$\vbox{\halign{\hfill # \qquad\hfill
&\hfill\qquad # \qquad\hfill&\hfill # \hfill\cr
\noalign{\hrule}
...
of the same form as (1), one for each fixed-total subgroup.
...
\[
f_{\bm{Y}}(\bm{y};{\bf p})=\displaystyle \left \{ \begin{array}{ll}
N_m!\prod_{i=1}^4 {{p_{mi}^{y_{mi}}}\over{y_{mi}!}}
N_f!\prod_{i=1}^4 {{p_{fi}^{y_{fi}}}\over{y_{fi}!}} &
\qquad\quad {\rm if} \displaystyle\sum_{i=1}^4 y_{mi}=N_m {\rm and} 
\displaystyle\sum_{i=1}^4 y_{fi}=N_f; \\
0 & {\rm otherwise},
\end{array}
\right.
\]
...
Using similar results to those used in \S3.2 (but not proved here), we can
...
Therefore
$$\begin{array}{ll}
P(R=j,C=k)&=\exp[\alpha+\beta_R(j)+\beta_C(k)-\log N]\cr
