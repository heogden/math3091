...
The linear model considered in Section \@ref(sn:lm) is also a generalised linear
...
the  Bernoulli distribution. The objective is to model
the success probability $p$ as a function of the explanatory variables.
...
of the logistic distribution taking values in the real line
$(-\infty < \eta < \infty)$. It can be easily verified that
$F(\eta) = \frac{1}{1+ \exp(-\eta)}$ is a cdf of a random variable since it is
non-negative and  increases  monotonically to 1 from zero. 
