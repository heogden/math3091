...
$$
\begin{array}{ll}
& \log f_Y(y;\theta,\phi)={{y\theta-b(\theta)}\over{a(\phi)}}
+c(y,\phi)\cr
\Rightarrow\quad
& u(\theta)=\frac{\partial}{\partial \theta}\log f_Y(y;\theta,\phi)
={{y-\frac{\partial}{\partial \theta} b(\theta)}\over{a(\phi)}}
={{y- b'(\theta)}\over{a(\phi)}}\cr
\Rightarrow\quad
&H(\theta)=\frac{\partial^2}{\partial \theta^2}\log f_Y(y;\theta,\phi)
...
the form \eqref{eqn:exponential_family}, are $b'(\theta)$ and $a(\phi)b''(\theta)$ respectively.
...
| Distribution               | Normal               | Poisson          | Binomial / Bernoulli              |
|----------------------------|----------------------|------------------|-----------------------------------|
| $b(\theta)$                | ${1\over 2}\theta^2$ | $\exp\theta$     | $\log(1+\exp\theta)$              |
| $b'(\theta)\equiv\mu$      | $\theta$             | $\exp\theta$     | $\frac{\exp\theta}{1+\exp\theta}$ |
| $b^{'-1}(\mu)\equiv\theta$ | $\mu$                | $\log\mu$        | $\log{\frac{\mu}{1-\mu}}$         |
| Link                       | $g(\mu)=\mu$         | $g(\mu)=\log\mu$ | $g(\mu)=\log{\frac{\mu}{1-\mu}}$  |
|                            | Identity link        | Log link         | Logistic (Logit) link             |
...
\begin{equation}
\log f_{\bm{Y}}(\bm{y};\bm{\beta},\bm{\phi})=
...
\log{f_{\bm{Y}}(\bm{y};\bm{\beta},\bm{\phi})}\qquad k=1,\ldots ,p
$$
and then
$$\begin{array}{l}
...
$$\begin{array}{ll}
u_k(\bm{\beta})&= {\partial\over{\partial\beta_k}}\log{f_{\bm{Y}}(\bm{y};\bm{\beta},\bm{\phi})}\cr
...
$$
[\bm{H}(\bm{\beta})]_{jk}={{\partial^2}\over{\partial\beta_j\partial\beta_k}}\log{f_{\bm{Y}}(\bm{y};\bm{\beta},\bm{\phi})}
...
&=2\log f_{\bm{Y}}(\bm{y};\hat{\bm{\theta}}^{(1)})-2\log f_{\bm{Y}}(\bm{y};\hat{\bm{\theta}}^{(0)}),  
  (\#eq:LRglm)
\end{align}
...
\begin{equation}
\log f_{\bm{Y}}(\bm{y};\bm{\theta})=\sum_{i=1}^n{{y_i\theta_i-b(\theta_i)}
...
$$
{\partial\over{\partial\theta_k}}\log f_{\bm{Y}}(\bm{y};\bm{\theta})={{y_k-b'(\theta_k)}
...
$$
L_{0s}=2\log f_{\bm{Y}}(\bm{y};\hat{\bm{\theta}}^{(s)})-2\log f_{\bm{Y}}(\bm{y};\hat{\bm{\theta}}^{(0)}),
$$
...
\begin{align}
L_{01}&=2\log f_{\bm{Y}}(\bm{y};\hat{\bm{\theta}}^{(1)})-2\log f_{\bm{Y}}(\bm{y};\hat{\bm{\theta}}^{(0)})\cr
&=[2\log f_{\bm{Y}}(\bm{y};\hat{\bm{\theta}}^{(s)})-2\log f_{\bm{Y}}(\bm{y};\hat{\bm{\theta}}^{(0)})]
-[2\log f_{\bm{Y}}(\bm{y};\hat{\bm{\theta}}^{(s)})-2\log f_{\bm{Y}}(\bm{y};\hat{\bm{\theta}}^{(1)})]\cr
&=L_{0s}-L_{1s}. 
  (\#eq:LRsaturated)
\end{align}
...
$$\begin{array}{ll}
%L_{0s}&=\sum_{i=1}^n y_i[\hat{\theta}^{(s)}_i-\hat{\theta}^{(0)}_i]
%-[b(\hat{\theta}^{(s)}_i)-b(\hat{\theta}^{(0)}_i)]\cr
