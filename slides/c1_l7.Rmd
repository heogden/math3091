...
## MSc programmes in Statistics at Southampton
Two programmes are currently available, offering high-level training in statistical methodology and applications.

- MSc Statistics
- MSc Statistics with Applications in Medicine

Both are 12 month programmes, with two semesters of taught modules and a summer project. 

Modules cover statistical theory, computing and modelling, with specialist modules in areas of medical statistics. 

For the MSc Statistics with Applications in Medicine, we have three NIHR scholarships to award (covers full fees and a stipend of \pounds15,000).
**Deadline 1 March 2019.**

Further details: \newline
\scriptsize
https://www.southampton.ac.uk/maths/postgraduate/taught_courses.page
...
In most of the models that we have looked at so far, we have
assumed that the responses $Y_i$ are independent and identically
distributed random variables, coming from some distribution known
up to a parameter $\bm \theta$. 

In regression models, the responses $Y_i$ will not be identically distributed:
the distribution of the response will depend on *explanatory variables*
$\bm x_i$.
...
## Linear model quiz 1: residual plot
...
## Linear model quiz 1: normal Q-Q plot
...
## Linear model quiz 2: simple linear residual plot

```{r, fig.width = 5, fig.height = 3.5}
mod2_simple <- lm(y ~ x)
plot(mod2_simple, which = 1)
```

## Linear model quiz 2: simple linear normal Q-Q plot

```{r, fig.width = 5, fig.height = 3.5}
plot(mod2_simple, which = 2)
```

## Linear model quiz 2: quadratic regression residual plot

```{r, fig.width = 5, fig.height = 3.5}
mod2_quad <- lm(y ~ x + I(x^2))
plot(mod2_quad, which = 1)
```

## Linear model quiz 2: quadratic regression normal Q-Q plot

```{r, fig.width = 5, fig.height = 3.5}
plot(mod2_quad, which = 2)
```

## Linear model quiz 3 ([web.meetoo.com](https://web.meetoo.com), 108-197-366)
```{r, echo = FALSE, fig.width = 5, fig.height = 4}
mu <- 0.5 + 2 * x 
sigma <- 0.1 + 0.5 * (x - min(x))
...
## Linear model quiz 3: residual plot
...
## Linear model quiz 3: normal Q-Q plot
...
- Next time, we'll look at how the likelihood can be used to do inference
on the parameters of the linear model, and compare this with the methods
of inference studied in MATH2010.
