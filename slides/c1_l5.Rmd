...
$U(\theta)$ is normally distributed. 
...
\pause
Now, $U(\hat{\theta})=0$, and if we approximate $U'(\theta) = H(\theta)$ by
...
The normal approximation is that $\hat p = \bar Y$ has approximately 
...
test hypotheses.
- Later in the course, we will use this to find confidence intervals
and test hypotheses for generalised linear models.
