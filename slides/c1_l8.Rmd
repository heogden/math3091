...
parameter $\beta_i$ equals zero (i.e. if $p - q = 1$), we could use a $t$ test 
...
So 
\[s.e.(\hat \beta_i) = \hat \sigma \sqrt{\left[(\bm{X}^T\bm{X})^{-1}\right]_{ii}},\]
and
\[t = \frac{\hat \beta_i}{s.e.(\hat \beta)} \sim t_{n-p}\]
under $H_0$ (that $\beta_i = 0$).
...
Please could you fill in the mid-semester feedback survey? 

You can access
this on the "surveys" tab at
[web.meetoo.com](https://web.meetoo.com),
meeting ID 108-197-366.

You can complete this now, or at any point up until Friday at 3pm.
