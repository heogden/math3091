...
- Next time, we will show that we make the same
conclusions about the relationships between variables by using
a Poisson log-linear model instead of a multinomial one
(provided that we include an intercept in the Poisson model).
- We'll also see an extension to the multinomial model in which one
