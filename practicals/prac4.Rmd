...
at the first  level of `poison`.  Which one is the first level of `poison`? Issue the command
```{r}
...
Once the above options command has been issued, it will set the first level of
all categorical factors to zero in the `lm` function.  The `contr.treatment`
has nothing to do with the treatment types in this example.
We would have used the same command even if the treatment in the data set
was called `drugs` instead.
