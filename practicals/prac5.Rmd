...
$$
The MLE of $\beta_2$ is $\hat{\beta_1} = 34.27$ with s.e.
(asymptotic standard error) $2.912$.  From this we can test $H_0: \beta_2 =0$
by noting that under $H_0$, approximately $\hat{\beta_1}/\text{s.e.} \sim N(0, 1)$. 
So we will reject $H_0$ if
$|z| = |\hat{\beta_1}/\text{s.e.}| > 1.96$. In this case, $z = 11.77$, so
