...
We can plot out these three link functions to compare them:
```{r}
z <- 1./(1. + exp( - x))
compl <- 1- exp(-exp(x))
curve(pnorm(x), from = -5, to = 5,
      main = "Comparison of Probit, Logit and Clog", col=2)
curve(1/(1 + exp(- x)), lty = 2, col = 4, add = TRUE)
curve(1- exp(-exp(x)), lty = 3, col = 6, add = TRUE)
legend("bottomleft", legend = c("Probit", "Logit", "cloglog"),
